    ...
    try {
        ...
        await prisma.$transaction(async (tx: any) => {
            await tx.agency.update({
                where: { id: agencyId },
                data: { booking_locked: locked }
            });

            await tx.auditLog.create({
                data: {
                    actor_id: userToken.userId,
                    action: locked ? 'BOOKING_LOCKED' : 'BOOKING_UNLOCKED',
                    entity: 'AGENCY_BOOKING',
                    entity_id: agencyId,
                }
            });
        });

        return res.status(200).json({ success: true, message: `Booking ${locked ? 'locked' : 'unlocked'}` });
